...
# - bis-rps             => Value : Rps
v_MBISRPMListing="Des"


# Main
##

##  Creating the list of services hosting on this host :
#
...
## Checking now the status of each services detected
#

echo "# HELP mbis_services the state of all mbis services on this host" > /tmp/node/mbis_services.prom
echo "# TYPE mbis_services gauge" >> /tmp/node/mbis_services.prom
...
do
        ...
        echo "mbis_services{service_name=\"${l_service}\"} ${v_ServiceState}" >> /tmp/node/mbis_services.prom
